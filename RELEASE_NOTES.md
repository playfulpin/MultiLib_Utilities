...
- `bin/populate_privetelib.sh` **1.1.0** — rebuild the app-registered personal library DB (`privetelib`) from the on-disk `Books` collection (Phase 1 of `docs/REPRESENTATION_PLAN.md`): every book file is md5-hashed (zip-wrapped FB2 by its **decompressed content**, loose `*.fb2` directly) and matched against `flibusta.mlbook.md5`, which the dump pipeline populates for ALL 869,130 catalog rows — so the match is exact and unambiguous.  **Only books present in the `Books` folder are represented**; v1.1.0 drops the v1.0.0 exact-copy approach (carried flibusta's ids wholesale, which broke the app's key bookkeeping) for **fresh-key, row-by-row population**: privetelib's own `AUTO_INCREMENT` columns generate every key, captured per row via `LAST_INSERT_ID()` into session variables (`@bid_`/`@aid_`/`@gid_`/`@sid_`) that the join tables (`mlauthor`, `mlgenre`, `mlseq`) and attached data (`mlrating`, `mlcustinfo`) reference — the whole rebuild is one SQL script in a single session (`TRUNCATE` first = clean rebuild).  Reference tables (`mlauthorname`, `mlgenrename`, `mlseqname`) are inserted for the personal library's books only, with `mlgenrename.parentgenreid` remapped to the fresh parent id (or `NULL` when the parent genre is not used); `mlbook.filename`/`arcname` carry the **real on-disk relative path** and zip member name (`library='privetelib'`, on-disk `filesize`, catalog metadata verbatim); `mlrating` is copied from `flibusta.mlrating` (the per-book aggregate produced by `BookTracker-import/sql/Flibusta_Load_mlrating.sql`) for books that have a rating.  `flibusta` is read-only, app-owned tables in `privetelib` are never touched, `mlcoverpage`/`mldescription` stay empty; a column-parity mismatch on ANY managed table aborts the run before any `TRUNCATE`; every run writes a matched/unmatched TSV report to `POP_REPORT_DIR`; shares the MariaDB lifecycle via `lib/mariadb_lifecycle.sh`, password via `MYSQL_PWD` only; mock suite `tests/test_populate_privetelib.sh` 31 assertions
