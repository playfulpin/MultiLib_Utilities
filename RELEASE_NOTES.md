...
- `bin/reconcile_library.sh` **1.0.1** — personal-catalog collection-progress report: the scope file (`data/fixtures/authors_list_from_db.txt`) is the recommended-author list, and the library root (default `/mnt/c/Backup_Go7/Books`) is where the user collects those authors' books; the report counts how much of the list is collected (`collected` / still-to-collect / empty) and what extra content sits beyond the list (the user's own picks, known / unknown to the catalog), with catalog book counts (mlauthorname snapshot) vs on-disk file counts, output as a progress summary plus a per-run TSV report
