...
# the REAL author list (authors_list_from_db.txt, 6,088 names):
#
...
#
# Coverage:
#   * STAGE 1 (generate) -- build_prefix_table.sh on the real list exits 0,
...
#   * STAGE 2 (validate) -- prefix_table_integrity.sh accepts the generated
#     table with exit 0, reports "0 critical", and checked the same row count
#     the generator emitted.
#   * STAGE 3 (render)  -- prefix_tree_visualizer.sh accepts the generated
...
# WSL instead:
#   wsl.exe bash test_e2e_pipeline.sh
#
...
GENERATOR="$SCRIPT_DIR/build_prefix_table.sh"
VALIDATOR="$SCRIPT_DIR/prefix_table_integrity.sh"
VISUALIZER="$SCRIPT_DIR/prefix_tree_visualizer.sh"
REAL_LIST="$SCRIPT_DIR/authors_list_from_db.txt"
...
if [[ ! -f "$REAL_LIST" ]]; then
    echo "SKIP  (authors_list_from_db.txt not found)"
    exit 0
fi
if [[ ! -f "$VALIDATOR" ]]; then
    echo "SKIP  (prefix_table_integrity.sh not found)"
    exit 0
fi
...
# STAGE 1: generate
###############################################################################
echo "== stage 1: generate (build_prefix_table.sh) =="
...
# STAGE 2: validate
###############################################################################
echo "== stage 2: validate (prefix_table_integrity.sh) =="
...
# STAGE 3: render
###############################################################################
echo "== stage 3: render (prefix_tree_visualizer.sh) =="
