...
#   Named-option style:
#
#       ./prefix_table_integrity.sh -t TABLE [-x MAX_PREFIX_LENGTH] [-s SEVERITY]
#
...
#
# EXAMPLES
#   ./prefix_table_integrity.sh tmp_SORTED_AUTHORS 5
#   ./prefix_table_integrity.sh --critical tmp_SORTED_AUTHORS 5
#   ./prefix_table_integrity.sh -t tmp_SORTED_AUTHORS -x 5 -s warnings
#
# -----------------------------------------------------------------------------
# SEVERITY LEVELS
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
usage() {
    echo "prefix_table_integrity.sh v$SCRIPT_VERSION"
    echo ""
...
check_table() {
    ...
    for (( i = 0; i < total_rows; i++ )); do
        ...
        # range-based tree walk in build_shell_nested_authors.sh DEPENDS on
