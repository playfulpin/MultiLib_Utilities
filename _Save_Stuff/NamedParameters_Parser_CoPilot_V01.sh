...
#
# Author:
# Version: 3.0 (with named arguments + debugger + format)
###############################################################################
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --input-file=*|-i=*|--min-authors=*|-m=*|--max-prefix=*|-x=*|--debugger=*|-d=*|--format=*|-f=*)
            flag="${1%%=*}"
            val="${1#*=}"
    ...
    case "$1" in
        ...
        -d|--debugger)
            ...
            case "$val" in
                ...
                0|off) DEBUGGER=0 ;;
                *)
                    echo "Error: --debugger expects ON/OFF or 1/0." >&2
                    exit 1
                    ;;
            esac
            shift 2
        ...
        -f|--format)
            ...
            case "$val" in
                ...
                shell) FORMAT="SHELL" ;;
                *)
                    echo "Error: --format expects SQL or SHELL." >&2
                    exit 1
                    ;;
            esac
            shift 2
