...
#   by prefix_table_integrity.sh (validator) and prefix_tree_visualizer.sh
#   (renderer).
#
# -----------------------------------------------------------------------------
...
#   Positional style:
#
#       ./build_prefix_table.sh <input_file> [<max_prefix_length>]
#
#   Named-option style (only the input file is required; -x defaults to 5):
#
#       ./build_prefix_table.sh -i INPUT_FILE [-x MAX_PREFIX_LENGTH] \
...
#
# EXAMPLES
#   ./build_prefix_table.sh alphabet_from_db.txt 5 > tmp_SORTED_AUTHORS
#   ./build_prefix_table.sh -i authors_list_clean_nfc.txt -x 5 -o tmp_SORTED_AUTHORS
#   ./build_prefix_table.sh -i authors_list_from_db.txt -x 5 -d on | head
#
# -----------------------------------------------------------------------------
# ALGORITHM (the sorted-range walk, from build_shell_nested_authors.sh)
# -----------------------------------------------------------------------------
#   1. NORMALIZE + SORT
...
# default used by build_shell_nested_authors.sh and prefix_table_integrity.sh.
...
# -----------------------------------------------------------------------------
usage() {
    echo "build_prefix_table.sh v$SCRIPT_VERSION"
    echo ""
...
# the same sorted-range recursion that build_shell_nested_authors.sh uses to
...
process_prefix() {
    ...
    for (( i = range_start; i < range_end; )); do
        ...
        # build_shell_nested_authors.sh, which skips word boundaries when
        # building directories.  The table lists every prefix, so "де " is a
        # valid row (and prefix_table_integrity.sh reports it as a warning).
...
main() {
    ...
    echo "build_prefix_table.sh v$SCRIPT_VERSION ($WALKER_VARIANT)" >&2
