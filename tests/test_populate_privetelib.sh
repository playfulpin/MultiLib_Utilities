...
#   - real on-disk paths: mlbook.filename = relative path under
#     POP_LIBRARY_ROOT, arcname = zip member name, library='privetelib'
#   - reference tables (mlauthorname, mlgenrename, mlseqname) populated
#     for the personal library's books only; mlgenrename parentgenreid
#     remapped to the fresh parent id (NULL when the parent is not used)
...
{
    printf '111\tflibusta\tTitle One\tru\t2024-01-14 16:55:25\t111\t890489\t\tfb2\t0\t%s\t\t\t\t\t\t\t\t\t\t1.1\t\t\t\t2022\t\n' "$zip1_md5"
    printf '222\tflibusta\tTitle Two\tru\t2024-02-14 16:55:25\t222\t890490\t\tfb2\t0\t%s\t\t\t\t\t\t\t\t\t\t1.1\t\t\t\t2021\t\n' "$zip2_md5"
    printf '333\tflibusta\tКнига Три\tru\t2024-03-14 16:55:25\t333\t890491\t\tfb2\t0\t%s\t\t\t\t\t\t\t\t\t\t1.1\t\t\t\t2020\t\n' "$fb2_md5"
} > "$FIX/mlbook.tsv"
...
# NormalCount - 9002's parent (9001) IS used; 9003's parent (7777) is NOT
{
...
if grep -q "SET @gid_9001 = LAST_INSERT_ID();" "$MOCK_SQL_LOG" \
   ...
   && grep -q "VALUES (NULL,'sf','Science Fiction',10,8);" "$MOCK_SQL_LOG"; then
    report "genres_parent_remap" ok
else
    report "genres_parent_remap" fail "sql=$(grep -E 'mlgenrename|@gid_' "$MOCK_SQL_LOG" 2>/dev/null | tr '\n' '|')"
fi
...
# mlbook: one row per resolved book with REAL on-disk paths + fresh @bid_
if grep -q "INSERT INTO privetelib.mlbook (library,title,lang,date_in,filename,filesize,arcname,ext,deleted,md5" "$MOCK_SQL_LOG" \
   && grep -q "'privetelib','Title One','ru','2024-01-14 16:55:25','A/Author One/Series X/01-Book One.zip'," "$MOCK_SQL_LOG" \
   && grep -q "'01-Book One.fb2','fb2'" "$MOCK_SQL_LOG" \
   && grep -q "'Книга Три'" "$MOCK_SQL_LOG" \
   ...
   && grep -q "SET @bid_333 = LAST_INSERT_ID();" "$MOCK_SQL_LOG"; then
    report "mlbook_real_paths_fresh_keys" ok
else
    report "mlbook_real_paths_fresh_keys" fail "sql=$(grep -E 'mlbook|@bid_' "$MOCK_SQL_LOG" 2>/dev/null | tr '\n' '|')"
fi
